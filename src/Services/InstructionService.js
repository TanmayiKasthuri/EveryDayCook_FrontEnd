...
const InstructionService = {
    ...
    async getInstructionsByRecipeId(id)
    {
        console.log(id)
        console.log(API_URL+id+API_URL_SUFFIX)
        try{
            const response=await fetch(API_URL+id+"/instructions"+API_URL_SUFFIX);
            if (!response.ok) {
          ...
          console.error(`Failed to fetch instructions for recipe id - ${id}:`, error);
          return [];
        }
